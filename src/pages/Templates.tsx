...
const META_STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  ...
  IN_REVIEW:  { label: 'Em revisão',  className: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
  REJECTED:   { label: 'Rejeitado',   className: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
  DISABLED:   { label: 'Desativado',  className: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400' },
  PAUSED:     { label: 'Pausado',     className: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' },
  IN_APPEAL:  { label: 'Em recurso',  className: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' },
  LIMIT_EXCEEDED: { label: 'Limite excedido', className: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' },
};

function MetaStatusBadge({ status }: { status: string }) {
  const cfg = META_STATUS_CONFIG[status] ?? { label: status, className: 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400' };
  return (
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Templates de Mensagem</h1>
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {metaLoading ? (
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Buscando templates da Meta...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum template encontrado na Meta</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Prévia do corpo</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Idioma</th>
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        {local?.createdAt && (
                          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Local: {formatDate(local.createdAt)}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">{t.language}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => handleDisparar(t, local?.midia)}
                            disabled={t.status !== 'APPROVED'}
                            className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
